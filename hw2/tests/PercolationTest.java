...
public class PercolationTest {
    ...
    public void yourFirstTestHere() {
        ...
        Cell[][] expectedState1 = {
                ...
                {Cell.CLOSED, Cell.CLOSED, Cell.CLOSED, Cell.FULL, Cell.FULL},
                {Cell.CLOSED, Cell.CLOSED, Cell.CLOSED, Cell.CLOSED, Cell.FULL}
        };
        assertThat(getState(N1, p1)).isEqualTo(expectedState1);
        assertThat(p1.percolates()).isTrue();

        int N2 = 5;
        Percolation p2 = new Percolation(N2);
        int[][] openGrids = {
                {0, 2},
                {0, 3},
                {1, 0},
        ...
        for (int[] site : openGrids) {
            p2.open(site[0], site[1]);
        }
        assertThat(getState(N2, p2)).isEqualTo(expectedState2);
        assertThat(p2.percolates()).isTrue();
